...
import '../TicketCard/TicketCard.css'
...
const TicketCard = ({ ticket, user }) => {
  ...
  const truncatedTitle = ticket.title.length > 70 ? ticket.title.slice(0, 70) + '...' : ticket.title;
  return (
    <div className="ticket-card-component">
      <div className='ticket-card-ticketId-holder'>
        {ticket.id}
      </div>
      <div className="ticket-card-title-holder">
        <span className="ticket-card-status-icon">
          {statusIcons[ticket.status] || 'No Status'}
        </span>
        <span className='ticket-card-title'>{truncatedTitle}</span>
      </div>
      <div className="ticket-card-priority">
        <div className="ticket-card-priority-icon">
          {priorityIcons[ticket.priority] || 'No Priority'}
        </div>
